...
def main(mpi=False, n_walkers=None, n_burn=256, n_iterations=None, overwrite=False):
    ...
    sampler_path = "only_sky_sampler.pickle"
    data_path = "only_sky_data.pickle"
    ...
    # pl.scatter(rot_rep.lon.degree, rot_rep.lat.degree)
    # pl.show()
    # return
    ...
    idx = rot_rep.lon.argmin()
    p0_guess = [rot_rep.lat.radian[idx],
                rot_rep.distance.decompose(galactic).value[idx],
                stream_v[0].decompose(galactic).value[idx],
                stream_v[1].decompose(galactic).value[idx],
                stream_v[2].decompose(galactic).value[idx]]
